...
class UnConvertNFePHP
{
     ...
     * @name nfexml2txt
     * @param
     *            mixed string ou array $arq Paths dos arquivos xmls
    ...
    private function cxtt($dom)
    {
        ...
        $vTotTrib = ! empty($ICMSTot->getElementsByTagName("$vTotTrib")->item(0)->nodeValue) ? $ICMSTot->getElementsByTagName("$vTotTrib")->item(0)->nodeValue : '';
        if ($vTotTrib == '') {
        ...
        if (isset($transp)) {
            ...
            if (isset($reboque)) {
                foreach ($reboque as $n => $reb) {
                    $placa = ! empty($reboque->item($n)
                        ->getElementsByTagName("placa")
                        ->item(0)->nodeValue) ? $reboque->item($n)
                        ->getElementsByTagName("placa")
                        ->item(0)->nodeValue : '';
                    $UF = ! empty($reboque->item($n)
                        ->getElementsByTagName("UF")
                        ->item(0)->nodeValue) ? $reboque->item($n)
                        ->getElementsByTagName("UF")
                        ->item(0)->nodeValue : '';
                    $RNTC = ! empty($reboque->item($n)
                        ->getElementsByTagName("RNTC")
                        ->item(0)->nodeValue) ? $reboque->item($n)
                        ->getElementsByTagName("RNTC")
                        ->item(0)->nodeValue : '';
                    // X22|Placa|UF|RNTC|
            ...
            if (isset($vol)) {
                foreach ($vol as $n => $volumes) {
                    ...
                    $qVol = ! empty($vol->item($n)
                        ->getElementsByTagName("qVol")
                        ->item(0)->nodeValue) ? $vol->item($n)
                        ->getElementsByTagName("qVol")
                        ->item(0)->nodeValue : '';
                    $esp = ! empty($vol->item($n)
                        ->getElementsByTagName("esp")
                        ->item(0)->nodeValue) ? $vol->item($n)
                        ->getElementsByTagName("esp")
                        ->item(0)->nodeValue : '';
                    $marca = ! empty($vol->item($n)
                        ->getElementsByTagName("marca")
                        ->item(0)->nodeValue) ? $vol->item($n)
                        ->getElementsByTagName("marca")
                        ->item(0)->nodeValue : '';
                    $nVol = ! empty($vol->item($n)
                        ->getElementsByTagName("nVol")
                        ->item(0)->nodeValue) ? $vol->item($n)
                        ->getElementsByTagName("nVol")
                        ->item(0)->nodeValue : '';
                    $pesoL = ! empty($vol->item($n)
                        ->getElementsByTagName("pesoL")
                        ->item(0)->nodeValue) ? $vol->item($n)
                        ->getElementsByTagName("pesoL")
                        ->item(0)->nodeValue : '';
                    $pesoB = ! empty($vol->item($n)
                        ->getElementsByTagName("pesoB")
                        ->item(0)->nodeValue) ? $vol->item($n)
                        ->getElementsByTagName("pesoB")
                        ->item(0)->nodeValue : '';
                    $lacres = $vol->item($n)
                        ->getElementsByTagName("lacres")
                        ->item(0);
                    $txt .= "X26|$qVol|$esp|$marca|$nVol|$pesoL|$pesoB|\r\n";
                    // monta dados dos lacres utilizados
                    if (isset($lacres)) {
                        foreach ($lacres as $n => $lac) {
                            $nLacre = ! empty($lacres->item($n)
                                ->getElementsByTagName("nLacre")
                                ->item(0)->nodeValue) ? $lacres->item($n)
                                ->getElementsByTagName("nLacre")
                                ->item(0)->nodeValue : '';
                            // X33|NLacre|
                            $txt .= "X33|$nLacre|\r\n";
        ...
        if (isset($cobr)) {
            ...
            if (isset($dup)) {
                foreach ($dup as $n => $duplicata) {
                    // Y07|NDup|DVenc|VDup|
                    $nDup = ! empty($dup->item($n)
                        ->getElementsByTagName("nDup")
                        ->item(0)->nodeValue) ? $dup->item($n)
                        ->getElementsByTagName("nDup")
                        ->item(0)->nodeValue : '';
                    $dVenc = ! empty($dup->item($n)
                        ->getElementsByTagName("dVenc")
                        ->item(0)->nodeValue) ? $dup->item($n)
                        ->getElementsByTagName("dVenc")
                        ->item(0)->nodeValue : '';
                    $vDup = ! empty($dup->item($n)
                        ->getElementsByTagName("vDup")
                        ->item(0)->nodeValue) ? $dup->item($n)
                        ->getElementsByTagName("vDup")
                        ->item(0)->nodeValue : '';
        ...
        if (isset($infAdic)) {
            ...
            if (isset($obsCont)) {
                foreach ($obsCont as $n => $oC) {
                    // Z04|XCampo|XTexto|
                    $xCampo = ! empty($obsCont->item($n)
                        ->getElementsByTagName("xCampo")
                        ->item(0)->nodeValue) ? $obsCont->item($n)
                        ->getElementsByTagName("xCampo")
                        ->item(0)->nodeValue : '';
                    $xTexto = ! empty($obsCont->item($n)
                        ->getElementsByTagName("xTexto")
                        ->item(0)->nodeValue) ? $obsCont->item($n)
                        ->getElementsByTagName("xTexto")
                        ->item(0)->nodeValue : '';
            ...
            if (isset($obsFisco)) {
                foreach ($obsFisco as $n => $pR) {
                    // Z07|XCampo|XTexto|
                    $xCampo = ! empty($obsFisco->item($n)
                        ->getElementsByTagName("xCampo")
                        ->item(0)->nodeValue) ? $obsFisco->item($n)
                        ->getElementsByTagName("xCampo")
                        ->item(0)->nodeValue : '';
                    $xTexto = ! empty($obsFisco->item($n)
                        ->getElementsByTagName("xTexto")
                        ->item(0)->nodeValue) ? $obsFisco->item($n)
                        ->getElementsByTagName("xTexto")
                        ->item(0)->nodeValue : '';
            ...
            if (isset($procRef)) {
                foreach ($procRef as $n => $pR) {
                    // Z10|NProc|IndProc|
                    $nProc = ! empty($procRef->item($n)
                        ->getElementsByTagName("nProc")
                        ->item(0)->nodeValue) ? $procRef->item($n)
                        ->getElementsByTagName("nProc")
                        ->item(0)->nodeValue : '';
                    $indProc = ! empty($procRef->item($n)
                        ->getElementsByTagName("infProc")
                        ->item(0)->nodeValue) ? $procRef->item($n)
                        ->getElementsByTagName("infProc")
                        ->item(0)->nodeValue : '';
        ...
        if (isset($cana)) {
            ...
            if (isset($forDia)) {
                foreach ($forDia as $n => $pR) {
                    // ZC04|dia|qtde|
                    $dia = ! empty($forDia->item($n)
                        ->getElementsByTagName("dia")
                        ->item(0)->nodeValue) ? $forDia->item($n)
                        ->getElementsByTagName("dia")
                        ->item(0)->nodeValue : '';
                    $qtde = ! empty($forDia->item($n)
                        ->getElementsByTagName("qtde")
                        ->item(0)->nodeValue) ? $forDia->item($n)
                        ->getElementsByTagName("qtde")
                        ->item(0)->nodeValue : '';
                    $txt .= "ZC04|$dia|$qtde|\r\n";
            ...
            if (isset($deduc)) {
                foreach ($deduc as $n => $pR) {
                    // ZC10|xDed|vDed|
                    $xDed = ! empty($deduc->item($n)
                        ->getElementsByTagName("xDed")
                        ->item(0)->nodeValue) ? $deduc->item($n)
                        ->getElementsByTagName("xDed")
                        ->item(0)->nodeValue : '';
                    $vDed = ! empty($deduc->item($n)
                        ->getElementsByTagName("vDed")
                        ->item(0)->nodeValue) ? $deduc->item($n)
                        ->getElementsByTagName("vDed")
                        ->item(0)->nodeValue : '';
                    $txt .= "ZC10|$xDed|$vDed|\r\n";
    ...
    private function getItens($det)
    {
        ...
        foreach ($det as $d) {
            ...
            $infAdProd = ! empty($det->item($i)
                ->getElementsByTagName("infAdProd")
                ->item(0)->nodeValue) ? $det->item($i)
                ->getElementsByTagName("infAdProd")
                ->item(0)->nodeValue : '';
            ...
            $DI = $det->item($i)
                ...
                ->item(0);
            $veicProd = $det->item($i)
                ->getElementsByTagName("veicProd")
                ->item(0);
            $med = $det->item($i)
                ->getElementsByTagName("med")
                ->item(0);
            $arma = $det->item($i)
                ->getElementsByTagName("arma")
                ->item(0);
            $comb = $det->item($i)
                ->getElementsByTagName("comb")
                ->item(0);
            $i ++;
            ...
            if (isset($DI)) {
                foreach ($DI as $x => $k) {
                    $nDI = ! empty($DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue) ? $DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue : '';
                    $dDI = ! empty($DI->item($x)
                        ->getElementsByTagName("nDI")
                        ...
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue : '';
                    $xLocDesemb = ! empty($DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue) ? $DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue : '';
                    $UFDesemb = ! empty($DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue) ? $DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue : '';
                    $dDesemb = ! empty($DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue) ? $DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue : '';
                    $cExportador = ! empty($DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue) ? $DI->item($x)
                        ->getElementsByTagName("nDI")
                        ->item(0)->nodeValue : '';
                    ...
                    $adi = $DI->item($X)
                        ->getElementsByTagName("adi")
                        ->item(0);
                    if (isset($adi)) {
                        foreach ($adi as $y => $k) {
                            // I25|nAdicao|nSeqAdic|cFabricante|vDescDI|
                            $nAdicao = ! empty($adi->item($y)
                                ->getElementsByTagName("nAdicao")
                                ->item(0)->nodeValue) ? $adi->item($y)
                                ->getElementsByTagName("nAdicao")
                                ->item(0)->nodeValue : '';
                            $nSeqAdic = ! empty($adi->item($y)
                                ->getElementsByTagName("nSeqAdic")
                                ->item(0)->nodeValue) ? $adi->item($y)
                                ->getElementsByTagName("nSeqAdic")
                                ->item(0)->nodeValue : '';
                            $cFabricante = ! empty($adi->item($y)
                                ->getElementsByTagName("cFabricante")
                                ->item(0)->nodeValue) ? $adi->item($y)
                                ->getElementsByTagName("cFabricante")
                                ->item(0)->nodeValue : '';
                            $vDescDI = ! empty($adi->item($y)
                                ->getElementsByTagName("vDescDI")
                                ->item(0)->nodeValue) ? $adi->item($y)
                                ->getElementsByTagName("vDescDI")
                                ->item(0)->nodeValue : '';
            ...
            if (isset($med)) {
                foreach ($med as $x => $k) {
                    $nLote = ! empty($med->item($x)
                        ->getElementsByTagName("nLote")
                        ->item(0)->nodeValue) ? $med->item($x)
                        ->getElementsByTagName("nLote")
                        ->item(0)->nodeValue : '';
                    $qLote = ! empty($med->item($x)
                        ->getElementsByTagName("qLote")
                        ->item(0)->nodeValue) ? $med->item($x)
                        ->getElementsByTagName("qLote")
                        ->item(0)->nodeValue : '';
                    $dFab = ! empty($med->item($x)
                        ->getElementsByTagName("dFab")
                        ->item(0)->nodeValue) ? $med->item($x)
                        ->getElementsByTagName("dFab")
                        ->item(0)->nodeValue : '';
                    $dVal = ! empty($med->item($x)
                        ->getElementsByTagName("dVal")
                        ->item(0)->nodeValue) ? $med->item($x)
                        ->getElementsByTagName("dVal")
                        ->item(0)->nodeValue : '';
                    $vPMC = ! empty($med->item($x)
                        ->getElementsByTagName("vPMC")
                        ->item(0)->nodeValue) ? $med->item($x)
                        ->getElementsByTagName("vPMC")
                        ->item(0)->nodeValue : '';
            ...
            if (isset($arma)) {
                foreach ($arma as $x => $k) {
                    $tpArma = ! empty($arma->item($x)
                        ->getElementsByTagName("tpArma")
                        ->item(0)->nodeValue) ? $arma->item($x)
                        ->getElementsByTagName("tpArma")
                        ->item(0)->nodeValue : '';
                    $nSerie = ! empty($arma->item($x)
                        ->getElementsByTagName("nSerie")
                        ->item(0)->nodeValue) ? $arma->item($x)
                        ->getElementsByTagName("nSerie")
                        ->item(0)->nodeValue : '';
                    $nCano = ! empty($arma->item($x)
                        ->getElementsByTagName("nCano")
                        ->item(0)->nodeValue) ? $arma->item($x)
                        ->getElementsByTagName("nCano")
                        ->item(0)->nodeValue : '';
                    $descr = ! empty($arma->item($x)
                        ->getElementsByTagName("descr")
                        ->item(0)->nodeValue) ? $arma->item($x)
                        ->getElementsByTagName("descr")
                        ->item(0)->nodeValue : '';
            ...
            if (isset($comb)) {
              ...
              // lei da transparencia 12.741/12
              // Nota Técnica 2013/003
            $vTotTrib = ! empty($imposto->getElementsByTagName("vTotTrib")->item(0)->nodeValue) ? $imposto->getElementsByTagName("vTotTrib")->item(0)->nodeValue : '';
            if ($vTotTrib == '') {
                $txt .= "M|\r\n";
            } else {
                $txt .= "M|$vTotTrib\r\n";
