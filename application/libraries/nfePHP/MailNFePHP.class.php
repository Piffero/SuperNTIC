...
class MailNFePHP
{
     ...
     *
     * @param array $aConfig
     *            Matriz com os dados de configuração
     * @param number $mododebug
     *            (Optional) 1-SIM ou 0-NÃO (0 default)
     * @package NFePHP
     * @author Roberto L. Machado <linux dot rlm at gmail dot com>
     */
     ...
     *            passar uma string com o caminho completo para o arquivo XML
     * @param string $para
     *            Força o envio da comunicação apenas para o email indicado
     ...
     * @name sendNFe
     * @param string $docXML
     *            arquivo XML, é obrigatório
     * @param string $docPDF
     *            DANFE em formato PDF, se não quizer mandar o pdf deixe em branco
     * @param string $nomeXML
     *            Nome do arquivo XML, é obrigatório
     * @param string $nomePDF
     *            Nome do arquivo PDF
     * @param array $aMail
     *            Matriz com as informações necessárias para envio do email
     * @param boolean $auth
     *            Indica se é necessária a autenticação
     * @return boolean TRUE sucesso ou FALSE falha
     */
     ...
     *            Assunto
     * @param string $txt
     *            Corpo do email em txt
     * @param string $htmlMessage
     *            Corpo do email em html
     * @param string $fileXML
     *            path completo para o arquivo xml
     * @param string $filePDF
     *            path completo para o arquivo pdf
     * @param string $auth
     *            Flag da autorização requerida 1-Sim 0-Não
     ...
     * @param string $email
     *            Endereço de email a ser testado, podem ser passados vários
                    ...
                    if (! $flagFolder) {
                        ...
                        if (imap_createmailbox($objMail, imap_utf7_encode($stream))) {
                            ...
                            if (imap_mail_move($objMail, "$msgno:$msgno", "$this->mailIMAPbox.$anomes")) {
                                ...
                                $this->mailERROR .= 'A mensagem ' . $result[0]->msgno . ' enviada por ' . $result[0]->from . ' em ' . $result[0]->date . ' Assunto ' . $result[0]->subject . ". Foi movida para a caixa postal [ $anomes ] e o 
     ...
     *            Objeto da conexão IMAP
     * @param integer $message_number
     *            Numero de ordem da mensagem na pasta IMAP
