...
class NFSePHPGinfes extends ToolsNFePHP
{
     ...
     * @param boolean $save_files
     *            : atributo que visa o salvamento automático das informações
     * @return none
     */
    ...
    /**
     * Método responsável pela montagem do lote de serviços RPS.
     * Cada nota lote pode ter "n" notas.
     *
     * @param int $idLote
     *            : código sequencial para numeração do lote. Entretando, o lote definitivo passado ao Ginfes será
     *            composto por [ date("ym") . sprintf("%011s", $idLote) ] ;
     * @param array $aNFSeLote
     *            : array contendo objetos NFSePHPGinfesData()
     ...
     *
     * @param string $xml
     *            : contém string com xml a ser sanitizado
     ...
     *
     * @param string $soap
     *            : string contendo retorno da chamada __sendSOAPNFSe()
     ...
     *            : tag a ser procurada
     * @param string $xml
     *            : xml do conteúdo a ser extraído
     * @param string $i
     *            : ordem da tag dentro do xml
     ...
     * Envia lote RPS
     *
     * @param string $xmlLote
     *            : xml montado a partir do método montarLoteRps() ou xml conforme especificado pelo manual ginfes
     ...
     *            : numero da NFS-e específica que deseja buscar, pode ser omitido caso deseja buscar por data
     * @param string $cnpj
     *            : cnpj do emissor
     * @param string $im
     *            : inscrição municipal do emissor
     * @param string $dtinicial
     *            : data inicial para filtragem de notas por periodo (pode ser omitido)
     * @param string $dtfinal
     *            : data final do periodo (pode ser omitido caso a primeira data tbm seja)
     * @param string $cpfcnpj_tomador
     *            : para pequisar pelo cnpj do tomador (ainda não implementado)
     * @param string $im_tomador
     *            : para pesquisar pela inscrição municipal do tomador (ainda não implementado)
     * @return boolean
     */
     ...
     *
     * @param string $xml
     *            : contém os métodos xml
     * @param string $servico
     *            : serviço no qual foi baseado a consulta
     */
     ...
     *
     * @param string $numNFSe
     *            : número da nfs-e a ser emitida em pdf
     * @param string $aParser
     *            : array contendo valores para substituição de informações que não constam no xml
     */
