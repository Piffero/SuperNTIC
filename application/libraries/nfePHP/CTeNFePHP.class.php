...
class CTeNFePHP
{
     ...
     * @name validXML
     * @param string $docxml
     *            string contendo o arquivo xml a ser validado
     ...
     *
     * @name addProt
     * @param string $ctefile
     *            path completo para o arquivo contendo a CTe
     * @param string $protfile
     *            path completo para o arquivo contendo o protocolo
    ...
    public function statusServico($UF = '', $tpAmb = '', $modSOAP = '2')
    {
        ...
        if (isset($retorno)) {
            ...
            $aRetorno['dhRecbto'] = date('d/m/Y H:i:s', $this->__convertTime($doc->getElementsByTagName('dhRecbto')
                ->item(0)->nodeValue));
     ...
     * retornados podem não ser os mais atuais.
     * Não é recomendado seu uso ainda.
     *
     ...
     * @name sendLot
     * @param array $aCTe
     *            conhecimento de transporte em xml uma em cada campo do array unidimensional MAX 50
     * @param integer $id
    ...
    public function sendLot($aCTe, $id, $modSOAP = '2')
    {
        ...
        if ($retorno) {
            ...
            $aRetorno['dhRecbto'] = ! empty($doc->getElementsByTagName('dhRecbto')->item(0)->nodeValue) ? date("d/m/Y H:i", $this->__convertTime($doc->getElementsByTagName('dhRecbto')
                ->item(0)->nodeValue)) : '';
     ...
     * @param string $chave
     *            numero da chave da CTe de 44 digitos
     * @param string $tpAmb
     ...
     * @name cancelCT
     * @param string $id
     *            ID da CTe com 44 digitos (sem o CTe na frente dos numeros)
     * @param string $protId
     *            Numero do protocolo de aceitaçao do lote de CTe enviado anteriormente pelo SEFAZ
     * @param boolean $modSOAP
     *            1 usa __sendSOAP e 2 usa __sendSOAP2
    ...
    public function verifySignatureXML($conteudoXML, $tag)
    {
        ...
        $conteudoAssinado = str_replace(array(
    ...
    {
        return rtrim(chunk_split(str_replace(array(
            "\r",
            "\n"
        ), '', $cnt), 76, "\n"));
    } // Fim __splitLines
     ...
     *            Critério de seleção pode ser usados coringas como *-cte.xml
