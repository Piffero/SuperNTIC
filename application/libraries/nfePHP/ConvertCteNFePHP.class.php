 ...
 *
 */
class ConvertCteNFePHP
{ // implements ConvertCTePHP{
    /*
...
")+%xs", $string) !== 1) {
    ...
    private function _TXT2XML_processa_array($array_ctes, $output_string = true)
    {
        ...
        foreach ($array_ctes as $kcte => $v) {
            unset($dom, $CTe, $infCte, $ide, $COMPL, $FLUXO, $ENTREGA, $REM, $INFNF, $DEST, $VPREST, $IMP, $INFCTENORM, $INFCARGA, $CONTQT, $DOCANT, $INFDOC, $EMIDOCANT, $IDDOCANT, $INFMODAL, $RODO, $RODO_OCC, $RODO_VEIC, $COBR, $IDE_dhCont, $TMP_ADD_COMPL, $FLUXO_xDest, $FLUXO_xRota, $CONTQT_dPrev, $cur_version);
            ...
            foreach ($v as $v2) {
                ...
                if ($v2['TAG'] == 'CTE') {
                    ...
                    if ($v2['TAG'] == 'IDE' && ! isset($ide)) {
                        ...
                        foreach ($campos as $nome_campo) {
                            if ($nome_campo == 'CNPJ' || $nome_campo == 'CPF' || $nome_campo == 'IE' || $nome_campo == 'xFant' || $nome_campo == 'xCpl' || $nome_campo == 'email') {
                                if (empty($v2[$nome_campo]))
                                    continue;
                            }
                    ...
                    } elseif (($v2['TAG'] == 'SEMDATA' || $v2['TAG'] == 'COMDATA' || $v2['TAG'] == 'NOPERIODO' || $v2['TAG'] == 'SEMHORA' || $v2['TAG'] == 'COMHORA' || $v2['TAG'] == 'NOINTER') && isset($ENTREGA)) {
                        if ($v2['TAG'] == 'SEMDATA')
                            $tmp_grupo = $dom->createElement("semData");
                        if ($v2['TAG'] == 'COMDATA')
                            $tmp_grupo = $dom->createElement("comData");
                        if ($v2['TAG'] == 'NOPERIODO')
                            $tmp_grupo = $dom->createElement("noPeriodo");
                        if ($v2['TAG'] == 'SEMHORA')
                            $tmp_grupo = $dom->createElement("semHora");
                        if ($v2['TAG'] == 'COMHORA')
                            $tmp_grupo = $dom->createElement("comHora");
                        if ($v2['TAG'] == 'NOINTER')
                            $tmp_grupo = $dom->createElement("noInter");
                        foreach ($campos as $nome_campo)
                            $tmp_grupo->appendChild($dom->createElement($nome_campo, $v2[$nome_campo]));
                        ...
                        if ($v2['TAG'] == 'INFNFE')
                            $tmp_grupo = $dom->createElement("infNFe");
                        if ($v2['TAG'] == 'INFOUTROS')
                            $tmp_grupo = $dom->createElement("infOutros");
                        foreach ($campos as $nome_campo) {
                            if ($nome_campo == 'PIN' || // nfe
$nome_campo == 'descOutros' || $nome_campo == 'nDoc' || $nome_campo == 'dEmi' || $nome_campo == 'vDocFisc') {
                                if (empty($v2[$nome_campo]))
                                    continue;
                            }
                            $tmp_grupo->appendChild($dom->createElement($nome_campo, $v2[$nome_campo]));
                        ...
                        // campo 'email' não existe no TXT
                        foreach ($campos as $nome_campo) {
                            if ($nome_campo == 'CNPJ' || $nome_campo == 'CPF' || $nome_campo == 'xFant' || $nome_campo == 'xCpl' || $nome_campo == 'email' || $nome_campo == 'CEP') {
                                if (empty($v2[$nome_campo]))
                                    continue;
                            }
                            if ($nome_campo == 'cPais' || $nome_campo == 'xPais') {
                        ...
                        // campo 'email' não existe no TXT
                        foreach ($campos as $nome_campo) {
                            if ($nome_campo == 'CNPJ' || $nome_campo == 'CPF' || $nome_campo == 'xFant' || $nome_campo == 'xCpl' || $nome_campo == 'email' || $nome_campo == 'CEP' || $nome_campo == 'ISUF' || $nome_campo == 'IE') {
                                if (empty($v2[$nome_campo]))
                                    continue;
                            }
                            if ($nome_campo == 'cPais' || $nome_campo == 'xPais') {
            ...
            if ($output_string) {
                ...
                $RETURN['xml'][$kcte] = str_replace('<?xml version="1.0" encoding="UTF-8  standalone="no"?>', '<?xml version="1.0" encoding="UTF-8"?>', $RETURN['xml'][$kcte]);
    ...
    private function __montaChaveXML(& $dom)
    {
        ...
        if (empty($dhEmi))
            return ("'dhEmi' não encontrado");
        $dhEmi = $dhEmi->nodeValue;
        ...
        $tempChave = substr(str_pad(abs((int) $cUF), 2, '0', STR_PAD_LEFT), 0, 2) . substr(str_pad(abs((int) $tempData[0] - 2000), 2, '0', STR_PAD_LEFT), 0, 2) . substr(str_pad(abs((int) $tempData[1]), 2, '0', STR_PAD_LEFT), 0, 2) . substr(str_pad($CNPJ, 14, '0', STR_PAD_LEFT), 0, 14) . substr(str_pad(abs((int) $mod), 2, '0', STR_PAD_LEFT), 0, 2) . substr(str_pad(abs((int) $serie), 3, '0', STR_PAD_LEFT), 0, 3) . substr(str_pad(abs((int) $nCT), 9, '0', STR_PAD_LEFT), 0, 9) . substr(str_pad(abs((int) $tpEmis), 1, '0', STR_PAD_LEFT), 0, 1) . substr(str_pad(abs((int) $cCT), 8, '0', STR_PAD_LEFT), 0, 8);
                ...
                if (! empty($errors)) {
                    foreach ($errors as $e) {
                        if ($e->level == LIBXML_ERR_ERROR || $e->level == LIBXML_ERR_FATAL)
                            $RETURN['erros'][$kcte][] = "$MSG_PADRAO " . $e->level . " " . $e->code . " - " . $e->message;
                        else
                            $RETURN['avisos'][$kcte][] = "$MSG_PADRAO " . $e->level . " " . $e->code . " - " . $e->message;
                    }
                }
                unset($errors, $e);
            } else {
