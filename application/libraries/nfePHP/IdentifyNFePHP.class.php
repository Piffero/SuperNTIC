    ...
    class IdentifyNFePHP
    {
        ...
        public function identifyBoletoContas($codigo_barra)
        {
            ...
            if (strlen($codigo_barra) == 44) {
                ...
                $dv_calc = $this->boletoModulo11CodigoBarra(substr($codigo_barra, 0, 4) . substr($codigo_barra, 5));
                if ($dv_calc == $dv && $dv_calc !== false) {
                    ...
                    if ($data_vencimento != '0000') {
                        $data_vencimento_ymd = ($data_vencimento - 1000) * 86400 + gmmktime(0, 0, 0, "07", "03", "2000") + 5000;
                ...
                if ($tipo_dv == '6' || $tipo_dv == '7' || $tipo_dv == '8' || $tipo_dv == '9') {
                    ...
                    if ($dv_calc == $dv && $dv_calc !== false || true) {
                        ...
                        $linha_digitavel = $p1 . $this->boletoModulo11CodigoBarra($p1) . ' ' . $p2 . $this->boletoModulo11CodigoBarra($p2) . ' ' . $p3 . $this->boletoModulo11CodigoBarra($p3) . ' ' . $p4 . $this->boletoModulo11CodigoBarra($p4);
                        $segmentos = array(
                            '1' => 'Prefeituras',
                            '2' => 'Saneamento',
        ...
        public function identifyChave($chave)
        {
            ...
            if (strlen($chave_numeros) == 44) {
                ...
                if (substr($chave_numeros, 20, 2) == '55' || substr($chave_numeros, 20, 2) == '57' || substr($chave_numeros, 20, 2) == '65') {
                    // verifica digito
        ...
        public function identifyFileTXT($parm)
        {
            ...
            if (is_string($parm)) {
                ...
                if (strpos($TMP_TXT, 'NOTA FISCAL|') === 0 || strpos($TMP_TXT, 'NOTAFISCAL|') === 0) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_TXT_NFE
                    ));
                }
                // REGISTROSCTE|1|
                // REGISTROS CTE|1|
                if (strpos($TMP_TXT, 'REGISTROSCTE|') === 0 || strpos($TMP_TXT, 'REGISTROS CTE|') === 0) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_TXT_CTE
                    ));
                }
                /*
                ...
                if (strpos($TMP_TXT, 'EMITENTE|') === 0) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_TXT_NFE_EMITENTE
                    ));
                }
                if (strpos($TMP_TXT, 'CLIENTE|') === 0) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_TXT_NFE_CLIENTE
                    ));
                }
                if (strpos($TMP_TXT, 'PRODUTO|') === 0) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_TXT_NFE_PRODUTO
                    ));
                }
                if (strpos($TMP_TXT, 'TRANSPORTADORA|') === 0) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_TXT_NFE_TRANSPORTADORA
                    ));
                }
                unset($TMP_TXT);
            }
        ...
        public function identifyFileXML($parm)
        {
            ...
            if (! is_object($parm)) { // pode ser passado uma instancia de DomDocument...
                ...
                if (! is_object($dom)) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_DESCONHECIDO
                    ));
                }
            } else {
                $dom = $parm;
            ...
            if (! empty($retDPEC) && ! empty($infDPECReg)) {
                ...
                if (substr($tmp_DPECReg, 0, 7) == 'RETDPEC' && strlen($tmp_DPECReg) == 21) { // RETDPEC07668027000104
                    if (! empty($infresCTe)) {
                        return (array(
                            'tipo' => NFEPHP_TIPO_ARQUIVO_DPEC_CTE
                        ));
                    }
                    if (! empty($infresNFe)) {
                        return (array(
                            'tipo' => NFEPHP_TIPO_ARQUIVO_DPEC_NFE
                        ));
                    }
                }
            }
            ...
            if (! empty($procEventoNFe)) {
                ...
                if (! empty($procnfeProcB2B) && ! empty($NFeB2B)) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_NFE_NFEB2B
                    ));
                }
                // verifica se não é nfeb2b
                ...
                if ($mod == 65) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_EVENTONFCE
                    ));
                }
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_EVENTONFE
                ));
            }
            // Eventos CTE
            ...
            if (! empty($procEventoCTe)) {
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_EVENTOCTE
                ));
            }
            ...
            if (! empty($CTe) && ! empty($infCTe)) {
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_CTE_SEM_PROTOCOLO
                ));
            }
            ...
            if (! empty($procCancCTe) && ! empty($cancCTe)) {
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_CTE_PROCCANCCTE
                ));
            }
            ...
            if (! empty($infInut) && ! empty($retInutCTe)) {
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_CTE_INUTFAIXA
                ));
            }
            unset($CTe, $infCTe, $protCTe, $procCancCTe, $cancCTe, $infInut, $retInutCTe);
            ...
            if (! empty($nfeProc)) {
                ...
                if ($mod == 65) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_NFCE
                    ));
                }
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_NFE
                ));
            }
            ...
            if (! empty($NFe) && ! empty($infNFe)) {
                ...
                if ($mod == 65) {
                    return (array(
                        'tipo' => NFEPHP_TIPO_ARQUIVO_NFCE_SEM_PROTOCOLO
                    )); // SERÁ QUE EXISTE?!
                }
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_NFE_SEM_PROTOCOLO
                ));
            }
            ...
            if (! empty($procCancNFe) && ! empty($cancNFe)) {
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_NFE_PROCCANCNFE
                ));
            }
            ...
            if (! empty($infInut) && ! empty($retInutNFe)) {
                return (array(
                    'tipo' => NFEPHP_TIPO_ARQUIVO_NFE_INUTFAIXA
                ));
            }
            unset($nfeProc, $NFe, $infNFe, $procCancNFe, $cancNFe, $infInut, $retInutNFe, $dom);
            return false;
        }
        ...
        {
            if (! function_exists('glob') || ! function_exists('exec') || ! function_exists('unlink') || ! function_exists('tempnam') || ! function_exists('file_get_contents') || ! function_exists('file_put_contents') || ! function_exists('escapeshellarg')) {
            ...
            if (! $force_imagem) {
                ...
                if ($tmp === false) {
                    ...
                    $cmd = escapeshellarg($this->path_convert) . " " . "-density 300 " . escapeshellarg($arquivo_tmp) . " " . escapeshellarg($arquivo_tmp . '.jpg');
                    // echo "convert=> $cmd\n";
                    exec($cmd);
                    @unlink($arquivo_tmp);
                    ...
                    if (is_array($GLOB)) {
                        foreach ($GLOB as $filename) {
                            ...
                            if ($tmp !== false) {
                                ...
                                if ($ret === false) {
                                    $ret = array(
                                        'tipo' => NFEPHP_TIPO_ARQUIVO_ARRAY
                                    );
                                }
                                $ret[] = $tmp;
                            }
            ...
            $cmd = escapeshellarg($this->path_zbarimg) . " " . "-D " . escapeshellarg($arquivo_tmp);
            // echo "zbarimg=> $cmd\n";
            @exec($cmd, $output);
            if (! $force_imagem) {
