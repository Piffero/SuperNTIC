...
class DamdfeNFePHP extends CommonNFePHP
{
    ...
    public function __construct($xmlfile = '', $sOrientacao = '', $sPapel = '', $sPathLogo = '', $sDestino = 'I', $sDirPDF = '', $fontePDF = '', $mododebug = 2)
    {
        ...
        if ($this->rodo->getElementsByTagName("valePed")->item(0) != "") {
            $this->valePed = $this->rodo->getElementsByTagName("valePed")
                ->item(0)
                ->getElementsByTagName("disp");
        }
        $this->chMDFe = str_replace('MDFe', '', $this->infMDFe->getAttribute("Id"));
    ...
    private function headerMDFePaisagem($x, $y, $pag)
    {
        ...
        $this->__textBox($x, $y, $w, 8, 'DAMDFE - Documento Auxiliar de Manifesto Eletronico de Documentos Fiscais', $aFont, 'T', 'C', 0, '');
    ...
    private function bodyMDFe($x, $y)
    {
        ...
        for ($i = 0; $i < $valesPedagios; $i ++) {
            $altura += 4;
            $texto = $this->valePed->item($i)
                ->getElementsByTagName('CNPJForn')
                ->item(0)->nodeValue;
            $aFont = array(
                'font' => $this->fontePadrao,
                'size' => 10,
        ...
        for ($i = 0; $i < $this->condutor->length; $i ++) {
            $y += 4;
            $texto = $this->condutor->item($i)
                ->getElementsByTagName('CPF')
                ->item(0)->nodeValue;
            $aFont = array(
                'font' => $this->fontePadrao,
                'size' => 10,
