...
class DacteNFePHP extends CommonNFePHP implements DocumentoNFePHP
{
     ...
     *            (Opcional) Orientação da impressão P ou L
     * @param string $sPapel
     *            Tamanho do papel (Ex. A4)
     * @param string $sPathLogo
     *            Caminho para o arquivo do logo
     * @param string $sDestino
     *            Estabelece a direção do envio do documento PDF
     * @param string $sDirPDF
     *            Caminho para o diretorio de armaz. dos PDF
     * @param string $fonteDACTE
     *            Nome da fonte a ser utilizada
     * @param number $mododebug
     *            0-Não 1-Sim e 2-nada (2 default)
     */
    public function __construct($docXML = '', $sOrientacao = '', $sPapel = '', $sPathLogo = '', $sDestino = 'I', $sDirPDF = '', $fonteDACTE = '', $mododebug = 2)
    {
     ...
     *
     * @param string $orientacao
     *            (Opcional) Estabelece a orientação da
        ...
        } else // Complementado
{
    ...
    protected function descricaoCargaDACTE($x = 0, $y = 0)
    {
        ...
        $this->__textBox($x + 1, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        $texto = $this->__simpleGetValue($this->infCarga, "vCarga") == "" ? $this->__simpleGetValue($this->infCarga, "vMerc") : $this->__simpleGetValue($this->infCarga, "vCarga");
        ...
        $texto .= number_format($this->__simpleGetValue($this->infQ->item(0), "qCarga") / $this->multiUniPeso($this->__simpleGetValue($this->infQ->item(0), "cUnid")), 3, ".", "");
        ...
        $texto .= number_format($this->__simpleGetValue($this->infQ->item(1), "qCarga") / $this->multiUniPeso($this->__simpleGetValue($this->infQ->item(1), "cUnid")), 3, ".", "");
    ...
    protected function compValorDACTE($x = 0, $y = 0)
    {
        ...
        foreach ($this->Comp as $k => $d) {
            $nome = $this->Comp->item($k)
                ->getElementsByTagName('xNome')
                ->item(0)->nodeValue;
            $valor = number_format($this->Comp->item($k)
                ->getElementsByTagName('vComp')
                ->item(0)->nodeValue, 2, ",", ".");
            if ($auxX > $w * 0.60) {
    ...
    protected function docOrigDACTE($x = 0, $y = 0)
    {
        ...
        foreach ($this->infNF as $k => $d) {
            $tp = $this->infNF->item($k)
                ->getElementsByTagName('mod')
                ->item(0)->nodeValue;
            $cnpj = $this->formatCNPJCPF($this->rem);
            $doc = $this->infNF->item($k)
                ->getElementsByTagName('serie')
                ->item(0)->nodeValue;
            $doc .= '/' . $this->infNF->item($k)
                ->getElementsByTagName('nDoc')
                ->item(0)->nodeValue;
            if ($auxX > $w * 0.90) {
                $yIniDados = $yIniDados + 4;
                $auxX = $oldX;
        ...
        foreach ($this->infNFe as $k => $d) {
            $tp = 'NFE';
            $chaveNFe = $this->infNFe->item($k)
                ->getElementsByTagName('chave')
                ->item(0)->nodeValue;
    ...
    protected function formatCNPJCPF($field)
    {
        ...
        if ($cnpj != "" && $cnpj != "00000000000000") {
            ...
            $cnpj = ! empty($field->getElementsByTagName("CPF")->item(0)->nodeValue) ? $this->__format($field->getElementsByTagName("CPF")
                ->item(0)->nodeValue, '###.###.###.###-##') : '';
        }
        return $cnpj;
    } // fim formatCNPJCPF
