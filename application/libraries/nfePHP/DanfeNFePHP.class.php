...
class DanfeNFePHP extends CommonNFePHP implements DocumentoNFePHP
{
     ...
     * @name __construct
     * @param string $docXML
     *            Arquivo XML da NFe (com ou sem a tag nfeProc)
     * @param string $sOrientacao
     *            (Opcional) Orientação da impressão P-retrato L-Paisagem
     * @param string $sPapel
     *            Tamanho do papel (Ex. A4)
     * @param string $sPathLogo
     *            Caminho para o arquivo do logo
     * @param string $sDestino
     *            Estabelece a direção do envio do documento PDF I-browser D-browser com download S-
     * @param string $sDirPDF
     *            Caminho para o diretorio de armazenamento dos arquivos PDF
     * @param string $fonteDANFE
     *            Nome da fonte alternativa do DAnfe
     * @param number $exibirPIS
     *            1-SIM e 0-Não
     * @param number $mododebug
     *            0-Não 1-Sim e 2-nada (2 default)
     */
     ...
     *            (Opcional) Estabelece a orientação da impressão
     *            (ex. P-retrato), se nada for fornecido será usado o padrão da NFe
     * @param string $papel
     *            (Opcional) Estabelece o tamanho do papel (ex. A4)
    ...
    public function montaDANFE($orientacao = '', $papel = 'A4', $logoAlign = 'C', $situacao_externa = NFEPHP_SITUACAO_EXTERNA_NONE, $CLASSE_PDF = false, $DPEC_NUMERO_REGISTRO = '')
    {
        ...
        if (isset($this->ISSQNtot)) {
            $itemTemp = ! empty($this->ISSQNtot->getElementsByTagName("vServ")->item(0)->nodeValue) ? $this->ISSQNtot->getElementsByTagName("vServ")->item(0)->nodeValue : 0;
            if ($itemTemp > 0) {
                $linhaISSQN = 1;
            }
        }
        ...
        if (isset($this->infAdic)) {
            ...
            $this->textoAdic .= ! empty($this->infAdic->getElementsByTagName("infCpl")->item(0)->nodeValue) ? 'Inf. Contribuinte: ' . trim($this->__anfavea($this->infAdic->getElementsByTagName("infCpl")
                ->item(0)->nodeValue)) : '';
            ...
            if (isset($obsCont)) {
                foreach ($obsCont as $obs) {
                    ...
                    $xTexto = ! empty($obsCont->item($i)
                        ->getElementsByTagName("xTexto")
                        ->item(0)->nodeValue) ? $obsCont->item($i)
                        ->getElementsByTagName("xTexto")
                        ->item(0)->nodeValue : '';
                    $this->textoAdic .= "\r\n" . $campo . ':  ' . trim($xTexto);
                    $i ++;
     ...
     * @name printDANFE
     * @param string $nome
     *            Path completo com o nome do arquivo pdf
     * @param string $destino
     *            Direção do envio do PDF
     * @param string $printer
     *            Identificação da impressora no sistema
    ...
    protected function __notaCancelada()
    {
        ...
        return $cStat == '101' || $cStat == '151' || $cStat == '135' || $cStat == '155' || $this->situacao_externa == NFEPHP_SITUACAO_EXTERNA_CANCELADA;
    }
     ...
     *            Número da Página
     * @param
     *            number$totPag Total de páginas
        ...
        if (($this->tpEmis == 2 || $this->tpEmis == 5) && ! $this->__notaDPEC()) {
            ...
            if ($this->__notaDPEC()) {
                ...
                if (isset($this->nfeProc)) {
                    ...
                    $tsHora = $this->__convertTime($this->nfeProc->getElementsByTagName("dhRecbto")
                        ->item(0)->nodeValue);
                    if ($texto != '') {
    ...
    protected function __destinatarioDANFE($x = 0, $y = 0)
    {
        ...
        if (! empty($this->dest->getElementsByTagName("CNPJ")->item(0)->nodeValue)) {
            $texto = $this->__format($this->dest->getElementsByTagName("CNPJ")
                ->item(0)->nodeValue, "###.###.###/####-##");
        } else {
            $texto = ! empty($this->dest->getElementsByTagName("CPF")->item(0)->nodeValue) ? $this->__format($this->dest->getElementsByTagName("CPF")
                ->item(0)->nodeValue, "###.###.###-##") : '';
        }
        $aFont = array(
            'font' => $this->fontePadrao,
        ...
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 1, '');
        $texto = $this->__ymd2dmy($this->ide->getElementsByTagName("dEmi")
            ->item(0)->nodeValue);
        $aFont = array(
            'font' => $this->fontePadrao,
            'size' => 10,
        ...
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 1, '');
        $texto = ! empty($this->dest->getElementsByTagName("fone")->item(0)->nodeValue) ? $this->__format($this->dest->getElementsByTagName("fone")
            ->item(0)->nodeValue, '(##) ####-####') : '';
        $aFont = array(
            'font' => $this->fontePadrao,
            'size' => 10,
     ...
     * Calcula o espaço ocupado pelo texto da fatura.
     * Este espaço só é utilizado quando não houver duplicata.
     *
    ...
    protected function __faturaDANFE($x, $y)
    {
        ...
        if ($this->dup->length > 0 || $textoFatura !== "") {
            ...
            foreach ($this->dup as $k => $d) {
                $nDup = ! empty($this->dup->item($k)
                    ->getElementsByTagName('nDup')
                    ->item(0)->nodeValue) ? $this->dup->item($k)
                    ->getElementsByTagName('nDup')
                    ->item(0)->nodeValue : '';
                $dDup = ! empty($this->dup->item($k)
                    ->getElementsByTagName('dVenc')
                    ->item(0)->nodeValue) ? $this->__ymd2dmy($this->dup->item($k)
                    ->getElementsByTagName('dVenc')
                    ->item(0)->nodeValue) : '';
                $vDup = ! empty($this->dup->item($k)
                    ->getElementsByTagName('vDup')
                    ->item(0)->nodeValue) ? 'R$ ' . number_format($this->dup->item($k)
                    ->getElementsByTagName('vDup')
                    ->item(0)->nodeValue, 2, ", ", ".") : '';
                $h = 8;
                $texto = '';
    ...
    protected function __transporteDANFE($x, $y)
    {
        ...
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 1, '');
        if (isset($this->transporta)) {
            $texto = ! empty($this->transporta->getElementsByTagName("CNPJ")->item(0)->nodeValue) ? $this->__format($this->transporta->getElementsByTagName("CNPJ")
                ->item(0)->nodeValue, "##.###.###/####-##") : '';
            if ($texto == '') {
                $texto = ! empty($this->transporta->getElementsByTagName("CPF")->item(0)->nodeValue) ? $this->__format($this->transporta->getElementsByTagName("CPF")
                    ->item(0)->nodeValue, "###.###.###-##") : '';
            }
        } else {
            $texto = '';
    ...
    protected function __descricaoProduto($itemProd)
    {
        ...
        $infAdProd = substr(! empty($itemProd->getElementsByTagName('infAdProd')->item(0)->nodeValue) ? $this->__anfavea($itemProd->getElementsByTagName('infAdProd')
            ->item(0)->nodeValue) : '', 0, 500);
        if (! empty($infAdProd)) {
     ...
     * Coloca o grupo de dados adicionais da NFe.
     * (retrato e paisagem)
     *
     ...
     *            Posição horizontal canto esquerdo
     * @param number $yFinal
     *            Posição vertical final para impressão
     */
     ...
     * Devolve uma string contendo informação sobre as notas referenciadas.
     * Suporta N notas, eletrônicas ou não
