...
class ConvertNfeNFePHP { // implements ConvertNFePHP{
	...
	public function TXT2XML($txt, $output_string = true) {
		...
		$RETURN = array (
				...
				'avisos' => array (),
				'xml' => array () 
		); // erros de interpretação do arquivo
		if (is_file ( $txt ))
	...
	private function _TXT2XML_verifica_last_tag(& $last_tag_array, $last_tag, $cur_tag, & $RETURN) {
		if (! isset ( $last_tag_array [$cur_tag] ))
			return;
		if (! in_array ( $last_tag, $this->campos_v200_lasttag [$cur_tag] ) && ! is_array ( $this->campos_v200_lasttag [$cur_tag] ))
			$RETURN ['erros'] [] = "$MSG_PADRAO Ultima tag $last_tag, não esta em: " . implode ( ', ', $this->campos_v200_lasttag [$cur_tag] );
		return;
	}
...
;
		...
		} elseif (preg_match ( "%(?:" . "[\xC2-\xDF][\x80-\xBF]" . // non-overlong 2-byte
...
")+%xs", $string ) !== 1) {
		...
		foreach ( $string as $v ) {
			...
			if ($TAG === 'NOTAFISCAL' || $TAG === 'NOTA FISCAL') {
				...
				if ($TAG == 'H') {
					...
					// TAGS 'M'/'N'
					if (in_array ( substr ( $TAG, 0, 1 ), array (
							'N',
							'O',
							'P',
							'Q',
					...
					if (substr ( $TAG, 0, 1 ) == 'O' && $TAG != 'O') {
						...
						if (($TAG == 'O07' && isset ( $cur_nota_tags ['O08'] )) || ($TAG == 'O08' && isset ( $cur_nota_tags ['O07'] ))) {
					...
					if (substr ( $TAG, 0, 1 ) == 'Q' && $TAG != 'Q') {
						...
						if ($TAG == 'Q07' || $TAG == 'Q10') {
							...
							if (($TAG == 'Q07' && isset ( $cur_nota_tags ['Q10'] )) || ($TAG == 'Q10' && isset ( $cur_nota_tags ['Q07'] ))) {
					...
					if (substr ( $TAG, 0, 1 ) == 'R' && $TAG != 'R') {
						...
						if (($TAG == 'R02' && isset ( $cur_nota_tags ['R04'] )) || ($TAG == 'R04' && isset ( $cur_nota_tags ['R02'] ))) {
					...
					if (substr ( $TAG, 0, 1 ) == 'S' && $TAG != 'S') {
						...
						if ($TAG == 'S07' || $TAG == 'S09') {
							...
							if (($TAG == 'S07' && isset ( $cur_nota_tags ['S09'] )) || ($TAG == 'S09' && isset ( $cur_nota_tags ['S07'] ))) {
					...
					if (substr ( $TAG, 0, 1 ) == 'T' && $TAG != 'T') {
						...
						if (($TAG == 'T02' && isset ( $cur_nota_tags ['T04'] )) || ($TAG == 'T04' && isset ( $cur_nota_tags ['T02'] ))) {
				...
				} elseif (($TAG === 'C02' || $TAG === 'C02a') || ($TAG === 'F02' || $TAG === 'F02a') || ($TAG === 'G02' || $TAG === 'G02a')) {
					...
					if (($TAG === substr ( $TAG, 0, 1 ) . '02' && isset ( $cur_nota_tags [substr ( $TAG, 0, 1 ) . '02a'] )) || ($TAG === substr ( $TAG, 0, 1 ) . '02a' && isset ( $cur_nota_tags [substr ( $TAG, 0, 1 ) . '02'] ))) {
					...
					if (($TAG === 'E02' && isset ( $cur_nota_tags ['E03'] )) || ($TAG === 'E03' && isset ( $cur_nota_tags ['E02'] ))) {
						$RETURN ['erros'] [] = "$MSG_PADRAO TAG informada incorretamente, tags opcionais já informada.";
						continue;
					}
					// TAGS W
					...
					if ($TAG == 'X04' || $TAG == 'X05') {
						...
						if (($TAG === 'X04' && isset ( $cur_nota_tags ['X05'] )) || ($TAG === 'X05' && isset ( $cur_nota_tags ['X04'] ))) {
	...
	private function _TXT2XML_processa_array($array_notas, $output_string = true) {
		...
		foreach ( $array_notas as $knota => $v ) {
			...
			$cur_version = '';
			unset ( $NFe, $infNFe, $ZC01_cana, $Z_infAdic, $X26_vol, $X_transp, $W_total, $G_entrega, $F_retirada, $E_dest, $C_emit, $B_refNFP, $B_refNFP_CPF_CNPJ, $B_NFref, $B20a_IE, $ide, $H_det, $Y_cobr, $C_xNome, $C_IE, $G_xLgr, $X03_transporta, 

			$M_imposto, $I_prod, $I18_DI, $H_infAdProd, $T_COFINSST, $S05_COFINSOutr, $S_COFINS, $R_PISST, $Q05_PISOutr, $Q_PIS, $P_II, $O_IPI, $O07_IPITrib, $N_ICMS, $L01_comb, $I18_DI, $I_xPed );
			foreach ( $v as $v2 ) {
				if ($v2 ['TAG'] == 'A') {
					// CRIA NOTA
					...
					if ($v2 ['TAG'] == 'B' && ! isset ( $ide )) {
						...
						unset ( $M_imposto, $I_prod, $I18_DI, $H_infAdProd, $T_COFINSST, $S05_COFINSOutr, $S_COFINS, $R_PISST, $Q05_PISOutr, $Q_PIS, $P_II, $O_IPI, $O07_IPITrib, $N_ICMS, $L01_comb, $I18_DI, $I_xPed );
						...
						foreach ( $campos as $nome_campo ) {
							if ($nome_campo == 'vDescDI' && empty ( $v2 ['vDescDI'] ))
								continue;
							$tmp_grupo->appendChild ( $dom->createElement ( $nome_campo, $v2 [$nome_campo] ) );
						}
						...
						foreach ( $campos as $nome_campo ) {
							if ($nome_campo == 'CODIF' && empty ( $v2 ['CODIF'] ))
								continue;
							if ($nome_campo == 'qTemp' && empty ( $v2 ['qTemp'] ))
								continue;
						...
						if ($v2 ['TAG'] == 'N02')
							$tmp_icms = $dom->createElement ( "ICMS00" );
						elseif ($v2 ['TAG'] == 'N03')
							$tmp_icms = $dom->createElement ( "ICMS10" );
						elseif ($v2 ['TAG'] == 'N04')
							$tmp_icms = $dom->createElement ( "ICMS20" );
						elseif ($v2 ['TAG'] == 'N05')
							$tmp_icms = $dom->createElement ( "ICMS30" );
						elseif ($v2 ['TAG'] == 'N06')
							$tmp_icms = $dom->createElement ( "ICMS40" );
						elseif ($v2 ['TAG'] == 'N07')
							$tmp_icms = $dom->createElement ( "ICMS51" );
						elseif ($v2 ['TAG'] == 'N08')
							$tmp_icms = $dom->createElement ( "ICMS60" );
						elseif ($v2 ['TAG'] == 'N09')
							$tmp_icms = $dom->createElement ( "ICMS70" );
						elseif ($v2 ['TAG'] == 'N10')
							$tmp_icms = $dom->createElement ( "ICMS90" );
						elseif ($v2 ['TAG'] == 'N10a')
							$tmp_icms = $dom->createElement ( "ICMSPart" );
						elseif ($v2 ['TAG'] == 'N10b')
							$tmp_icms = $dom->createElement ( "ICMSST" );
						elseif ($v2 ['TAG'] == 'N10c')
							$tmp_icms = $dom->createElement ( "ICMSSN101" );
						elseif ($v2 ['TAG'] == 'N10d')
							$tmp_icms = $dom->createElement ( "ICMSSN102" );
						elseif ($v2 ['TAG'] == 'N10e')
							$tmp_icms = $dom->createElement ( "ICMSSN201" );
						elseif ($v2 ['TAG'] == 'N10f')
							$tmp_icms = $dom->createElement ( "ICMSSN202" );
						elseif ($v2 ['TAG'] == 'N10g')
							$tmp_icms = $dom->createElement ( "ICMSSN500" );
						elseif ($v2 ['TAG'] == 'N10h')
							$tmp_icms = $dom->createElement ( "ICMSSN900" );
						
						foreach ( $campos as $nome_campo ) {
							// CAMPO QUE CASO SEJAM EM BRANCO, NÃO ENTRAM NO XML:
							if ($v2 ['TAG'] == 'N03' || $v2 ['TAG'] == 'N05' || $v2 ['TAG'] == 'N09' || $v2 ['TAG'] == 'N10e' || $v2 ['TAG'] == 'N10f') {
								if (in_array ( $nome_campo, array (
										'pMVAST',
										'pRedBCST' 
								) )) { // sacanagem não informar...
									if (empty ( $v2 [$nome_campo] ))
										continue;
								}
							} elseif ($v2 ['TAG'] == 'N06') {
			...
			if ($output_string) {
				...
				$RETURN ['xml'] [$knota] = str_replace ( '<?xml version="1.0" encoding="UTF-8  standalone="no"?>', '<?xml version="1.0" encoding="UTF-8"?>', $RETURN ['xml'] [$knota] );
	...
	private function __montaChaveXML(& $dom) {
		...
		if (empty ( $ide ))
			return ("'ide' não encontrado");
		$emit = $dom->getElementsByTagName ( "emit" )->item ( 0 );
		if (empty ( $emit ))
			return ("'emit' não encontrado");
		$cUF = $ide->getElementsByTagName ( 'cUF' )->item ( 0 );
		if (empty ( $cUF ))
			return ("'cUF' não encontrado");
		$cUF = $cUF->nodeValue;
		$dEmi = $ide->getElementsByTagName ( 'dEmi' )->item ( 0 );
		if (empty ( $dEmi ))
			return ("'dEmi' não encontrado");
		$dEmi = $dEmi->nodeValue;
		...
		$tempChave = substr ( str_pad ( abs ( ( int ) $cUF ), 2, '0', STR_PAD_LEFT ), 0, 2 ) . substr ( str_pad ( abs ( ( int ) $tempData [0] - 2000 ), 2, '0', STR_PAD_LEFT ), 0, 2 ) . substr ( str_pad ( abs ( ( int ) $tempData [1] ), 2, '0', STR_PAD_LEFT ), 0, 2 ) . substr ( str_pad ( $CNPJ, 14, '0', STR_PAD_LEFT ), 0, 14 ) . substr ( str_pad ( abs ( ( int ) $mod ), 2, '0', STR_PAD_LEFT ), 0, 2 ) . substr ( str_pad ( abs ( ( int ) $serie ), 3, '0', STR_PAD_LEFT ), 0, 3 ) . substr ( str_pad ( abs ( ( int ) $nNF ), 9, '0', STR_PAD_LEFT ), 0, 9 ) . substr ( str_pad ( abs ( ( int ) $tpEmis ), 1, '0', STR_PAD_LEFT ), 0, 1 ) . substr ( str_pad ( abs ( ( int ) $cNF ), 8, '0', STR_PAD_LEFT ), 0, 8 );
		...
		foreach ( $xml as $knota => $tmp_xml ) {
				...
				if (! empty ( $errors )) {
					foreach ( $errors as $e ) {
						if ($e->level == LIBXML_ERR_ERROR || $e->level == LIBXML_ERR_FATAL)
							$RETURN ['erros'] [$knota] [] = "$MSG_PADRAO " . $e->level . " " . $e->code . " - " . $e->message;
						else
							$RETURN ['avisos'] [$knota] [] = "$MSG_PADRAO " . $e->level . " " . $e->code . " - " . $e->message;
					}
				}
				unset ( $errors, $e );
			} else {
			...
			if ($versao == '2.00') {
				...
				for($cur_item = 0; $cur_item < $det->length; $cur_item ++) {
					...
					$N = NULL;
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS00" );
						if (! empty ( $N ))
							$N = 'N02';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS10" );
						if (! empty ( $N ))
							$N = 'N03';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS20" );
						if (! empty ( $N ))
							$N = 'N04';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS30" );
						if (! empty ( $N ))
							$N = 'N05';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS40" );
						if (! empty ( $N ))
							$N = 'N06';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS51" );
						if (! empty ( $N ))
							$N = 'N07';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS60" );
						if (! empty ( $N ))
							$N = 'N08';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS70" );
						if (! empty ( $N ))
							$N = 'N09';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMS90" );
						if (! empty ( $N ))
							$N = 'N10';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMSPart" );
						if (! empty ( $N ))
							$N = 'N10a';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMSST" );
						if (! empty ( $N ))
							$N = 'N10b';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMSSN101" );
						if (! empty ( $N ))
							$N = 'N10c';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMSSN102" );
						if (! empty ( $N ))
							$N = 'N10d';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMSSN201" );
						if (! empty ( $N ))
							$N = 'N10e';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMSSN202" );
						if (! empty ( $N ))
							$N = 'N10f';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMSSN500" );
						if (! empty ( $N ))
							$N = 'N10g';
						else
							$N = NULL;
					...
					if (empty ( $N )) {
						$N = $imposto->getElementsByTagName ( "ICMSSN900" );
						if (! empty ( $N ))
							$N = 'N10h';
						else
							$N = NULL;
